package com.fernandez.foro_hub.infra.exceptions;
...
public class GestorDeErrores {
    ...
    public ResponseEntity gestionarError404() {
        return ResponseEntity.notFound().build();//Retorna un 404 cuando no encuentra el recurso (EntityNotFoundException).
    }
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity gestionarError400(MethodArgumentNotValidException ex) {
        var errores = ex.getFieldErrors();
        return ResponseEntity.badRequest().body(errores.stream().map(DatosErrorValidacion::new).toList());//Sin el .build(), por que el .body ya nos regresa el ResponseEntity;//Retorna un 404 cuando no encuentra el recurso (EntityNotFoundException).
    }
