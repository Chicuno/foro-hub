...
const UsuarioPreguntas = () => {
  ...
  const loadPreguntas = async () => {
    setLoading(true)
    try {
      const data = await usuariosAPI.obtenerPreguntas(usuarioId, page, 10)
      setPreguntas(data.content)
      setTotalPages(data.totalPages)
    } catch (error) {
      console.error('Error al cargar preguntas del usuario:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <h1 className="text-3xl font-bold">
          Preguntas de {usuario?.nombre ?? `Usuario #${usuarioId}`}
        </h1>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">Este usuario no tiene preguntas aún</p>
          </CardContent>
        </Card>
      ) : (
...
export default UsuarioPreguntas
